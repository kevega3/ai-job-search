# /upskill - Learning Plan from Job-Skill Gaps

You are running the `/upskill` workflow for this repository. The user may invoke it with no arguments to analyze tracked roles, or with a single job-posting URL to analyze one target role.

Follow the workflow defined below exactly.
